...
### Example: Action Button Plugin

```python
#!/usr/bin/env python3
"""
10_system_update.py - System update button (root only)
"""
import json
import os
...
**Key Features:**
- Uses `os.geteuid() == 0` to check if user is root
- Returns empty widgets array for non-root users (plugin won't display)
- Uses `DEBIAN_FRONTEND=noninteractive` to prevent prompts
- Full-width button with `gridWidth: 2`
- User must confirm action in app before execution
